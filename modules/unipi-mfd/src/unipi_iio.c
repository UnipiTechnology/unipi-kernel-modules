 ...
 */
static void float2int_with_divider(u16 low_val, u16 high_val, int factor,
...
unipi_iio_map_mode_resistance(struct iio_dev *indio_dev)
{
	struct unipi_iio_device *n_iio = iio_priv(indio_dev);
	switch (n_iio->mode) {
	case 4:
...
static int unipi_iio_ai_read_raw(struct iio_dev *indio_dev,
				 ...
				 int *val2, long mask)
{
	...
	if ((mask == IIO_CHAN_INFO_RAW) && (n_iio->raw_valreg >= 0)) {
		if ((mode != ch->type) &&
		    !((mode == IIO_TEMP) && (ch->type == IIO_RESISTANCE)))
			return -EINVAL;
		raw_value = 0;
		regmap_bulk_read(iio_platform->map, n_iio->raw_valreg,
				 &raw_value,
				 iio_platform->descriptor->raw_valsize);
		*val = raw_value;
	...
	regmap_bulk_read(iio_platform->map, n_iio->valreg, &float_as_u32,
			 AI_VAL_REG_COUNT);

	switch (mode) {
	case IIO_VOLTAGE: {
		if (ch->type == IIO_VOLTAGE) {
			float2int_with_divider(lo16(float_as_u32),
					       hi16(float_as_u32), 1000, val,
					       val2);
...
static int unipi_iio_ai_read_raw_u32(struct iio_dev *indio_dev,
				     struct iio_chan_spec const *ch, int *val,
				     int *val2, long mask)
{
	struct unipi_iio_device *n_iio = iio_priv(indio_dev);
	...
	if (mask == IIO_CHAN_INFO_RAW) {
		regmap_bulk_read(iio_platform->map, n_iio->valreg, &raw_value,
				 AI_VAL_REG_COUNT);
		*val = raw_value;
		return IIO_VAL_INT;
	}
...
static int unipi_iio_ao_read_raw(struct iio_dev *indio_dev,
				 struct iio_chan_spec const *ch, int *val,
				 int *val2, long mask)
{
	struct unipi_iio_device *n_iio = iio_priv(indio_dev);
...
static int unipi_iio_ao_write_raw(struct iio_dev *indio_dev,
				  struct iio_chan_spec const *ch, int val,
				  int val2, long mask)
{
	struct unipi_iio_device *n_iio = iio_priv(indio_dev);
...
static int unipi_iio_probe(struct platform_device *pdev)
{
	...
	iio_platform = devm_kzalloc(dev, sizeof(struct unipi_iio_platform),
				    GFP_KERNEL);
	if (!iio_platform)
		return -ENOMEM;
	...
	if (iio_platform->descriptor->raw_valsize > 0) {
		...
		if (ret != 0) {
			dev_err(dev,
				"Invalid raw-value-reg property in devicetree\n");
		}
	}
	...
	for (i = 0; i < iio_platform->io_count; i++) {
		iio_dev = devm_iio_device_alloc(
			dev, sizeof(struct unipi_iio_device));
		iio_dev->modes = INDIO_DIRECT_MODE;
		iio_dev->name = "aio_type_unipi";
		iio_dev->dev.parent = dev;
		dev_set_name(&iio_dev->dev, iio_platform->descriptor->fname,
			     iogroup->address, i + 1);
		...
		n_iio->valreg =
			io_valreg + (i * iio_platform->descriptor->valsize);
		if (raw_valreg < 0) {
			n_iio->raw_valreg = -1;
		} else {
			n_iio->raw_valreg =
				raw_valreg +
				(i * iio_platform->descriptor->raw_valsize);
		}
	...
	/*
          for (i=0; i < iio_platform->io_count; i++) {
                  snprintf(name, sizeof(name), iio_platform->descriptor->fname,
     iogroup->address, i+1); string_upper(name, name);
                  unipi_mfd_add_group(dev->parent, name, unipi_mfd_ai_attrs, 2,
                                  (u32) (io_modereg == -1) ? -1 : io_modereg +
     i, (u32) io_valreg + (i * iio_platform->descriptor->valsize));
          }
  */
	return 0;
}
...
__maybe_unused static int unipi_iio_remove(struct platform_device *pdev)
{
	/*
          int i;
          struct unipi_iio_platform *iio_platform = (struct unipi_iio_platform*)
     platform_get_drvdata(pdev); struct unipi_iogroup_device *iogroup =
     to_unipi_iogroup_device(pdev->dev.parent); char name[30];
