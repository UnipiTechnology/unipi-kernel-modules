						   ...
						   int len)
{
	...
	len += 2;
	trans_count =
		((len - 1) / NEURONSPI_MAX_TX) + 2; // number of transmissions

	message = kzalloc(sizeof(struct spi_message) +
...
int unipi_spi_firmware_op(void *proto_self, u8 *send_buf, u8 *recv_buf, s32 len,
			  int firmware_cookie)
{
	...
	if (n_spi->firmware_in_progress) {
		if (n_spi->firmware_in_progress != firmware_cookie) {
			...
			unipi_fw_error(spi_dev,
				       "Firmare operation already in-progress");
			return -EACCES;
		}
	} else {
