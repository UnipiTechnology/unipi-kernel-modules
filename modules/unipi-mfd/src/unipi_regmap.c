...
// #define UNIPI_REGMAP_GET_BUFFER_SIZE_1  (UNIPI_REGMAP_REGISTER_SIZE +
// UNIPI_MODBUS_HEADER_SIZE + UNIPI_MODBUS_TAIL_SIZE) #define
// UNIPI_REGMAP_GET_BUFFER_SIZE_2  (2*UNIPI_REGMAP_REGISTER_SIZE +
// UNIPI_MODBUS_HEADER_SIZE + UNIPI_MODBUS_TAIL_SIZE) #define
// UNIPI_REGMAP_GET_BUFFER_SIZE(regcount) ((regcount)*UNIPI_REGMAP_REGISTER_SIZE
// + UNIPI_MODBUS_HEADER_SIZE + UNIPI_MODBUS_TAIL_SIZE)
...
static int unipi_regmap_hw_read(void *context, const void *reg_buf,
				size_t reg_size, void *val_buf, size_t val_size)
{
	...
	regcount = (val_size + (UNIPI_REGMAP_REGISTER_SIZE - 1)) /
		   UNIPI_REGMAP_REGISTER_SIZE;
	// printk("unipi_regmap_hw_read r=%d rs=%ld vs=%ld rc=%d\n", *(u16*)reg_buf,
	// reg_size, val_size, regcount);
	if (unipi_read_regs_sync(channel, *(u16 *)reg_buf, regcount, buffer) !=
	    regcount)
		return -EINVAL;
...
static int unipi_regmap_hw_reg_read(void *context, unsigned int reg,
				    unsigned int *val)
{
	struct unipi_channel *channel = context;
	u16 buf[4];
...
static int unipi_regmap_hw_gather_write(void *context, const void *reg,
...
static int unipi_regmap_hw_write(void *context, const void *buf,
				 size_t buf_size)
{
	return unipi_regmap_hw_gather_write(context, buf, sizeof(u16),
					    (u8 *)buf + sizeof(u16),
					    buf_size - sizeof(u16));
}

static int unipi_regmap_hw_reg_write(void *context, unsigned int reg,
				     unsigned int val)
{
	struct unipi_channel *channel = context;
...
static int unipi_regmap_async_write(void *context, const void *reg,
				    size_t reg_len, const void *val,
				    size_t val_len, struct regmap_async *a)
{
	struct unipi_channel *channel = context;
	struct unipi_regmap_async *async =
		container_of(a, struct unipi_regmap_async, core);
	u8 *buffer;
	int ret, regcount;

	if ((reg_len != sizeof(u16)) && (val != NULL))
		return -EINVAL;
	if (val == NULL) {
	...
	regcount = (val_len + (UNIPI_REGMAP_REGISTER_SIZE - 1)) /
		   UNIPI_REGMAP_REGISTER_SIZE;
	// printk("REGMAP: reg=%d regcount=%d val_len=%d\n", *(u16*)reg, regcount,
	// val_len);
	ret = unipi_write_regs_async(channel, *(u16 *)reg, regcount, buffer,
				     async, unipi_regmap_complete);
	if (ret != 0)
		kfree(buffer);
	return ret;
}
...
static const struct regmap_bus unipi_regmap_bus = {
	.fast_io = 0,
	.write =
		unipi_regmap_hw_write, // must be defined else there is some error
	// in regmap
	...
	.reg_format_endian_default =
		REGMAP_ENDIAN_NATIVE, /* REGMAP_ENDIAN_LITTLE */
...
static int unipi_regmap_hw_read_coil(void *context, const void *reg_buf,
				     ...
				     size_t val_size)
{
	...
	if (unipi_read_bits_sync(channel, *(u16 *)reg_buf, 1, (u8 *)buffer) <
	    val_size) {
		return -EINVAL;
	}
...
static int unipi_regmap_hw_reg_read_coil(void *context, unsigned int reg,
					 unsigned int *val)
{
	struct unipi_channel *channel = context;
	u32 buf[2] = { 0, 0 };
...
static int unipi_regmap_hw_reg_write_coil(void *context, unsigned int reg,
					  unsigned int val)
{
	struct unipi_channel *channel = context;
...
static int unipi_regmap_hw_gather_write_coil(void *context, const void *reg,
					     size_t reg_len, const void *val,
					     size_t val_len)
{
	...
	ret = unipi_write_bits_sync(channel, *(u16 *)reg, val_len,
				    (u8 *)buffer);
	if (ret < 0)
		return ret;
	return 0;
...
static int unipi_regmap_hw_write_coil(void *context, const void *buf,
				      size_t buf_size)
{
	return unipi_regmap_hw_gather_write_coil(context, buf, sizeof(u16),
						 (u8 *)buf + sizeof(u16),
						 buf_size - sizeof(u16));
}

static int unipi_regmap_async_write_coil(void *context, const void *reg,
					 size_t reg_len, const void *val,
					 size_t val_len, struct regmap_async *a)
{
	/* Max coil count is 32 -> use buffer with 32 bits data
   ...
   */
	...
	async->buffer = NULL;
	ret = unipi_write_bits_async(channel, *(u16 *)reg, val_len,
				     (u8 *)buffer, async,
				     unipi_regmap_complete);
	if (ret < 0)
		return ret;
	return 0;
}
...
struct regmap *devm_regmap_init_unipi_regs(struct unipi_channel *channel,
					   const struct regmap_config *config)
{
	return devm_regmap_init(channel->dev, &unipi_regmap_bus, channel,
				config ? config : &unipi_regmap_config_default);
}

struct regmap *devm_regmap_init_unipi_coils(struct unipi_channel *channel,
					    const struct regmap_config *config)
{
	return devm_regmap_init(channel->dev, &unipi_regmap_coil_bus, channel,
				config ? config :
					 &unipi_regmap_coil_config_default);
}
