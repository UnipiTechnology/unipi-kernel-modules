...
{
	int ret = 0x20;
	if ((op == UNIPI_SPI_OP_WRITEREG) || (op == UNIPI_SPI_OP_WRITEBITS))
		ret |= 0x10;
	...
	if ((op == UNIPI_SPI_OP_WRITEREG) || (op == UNIPI_SPI_OP_READREG)) {
		...
		if (reg < 500)
			return -EIO;
		if (reg < 755)
			return ret | 0x1 | (((reg - 500)) << 8);
		if (reg < 1000)
			return -EIO;
		if (reg < 1255)
			return ret | 0x2 | (((reg - 1000)) << 8);
	} else {
		if (reg < 64)
			return ret | 0x4 | (reg << 8);
		if (reg < 1000)
			return -EIO;
		if (reg < 1016)
			return ret | 0x5 | (((reg - 1000) & 0x3f) << 8);
		if (reg < 1080)
			return ret | 0x6 | (((reg - 1016) & 0x3f) << 8);
	}
	return -EIO;
}

static int unipi_spi2_parse_frame(struct unipi_spi_context *context,
				  ...
				  struct unipi_spi_reply *reply)
{
	...
	unipi_spi_trace_1(context->message.spi,
			  "RESP(rqop=%02x)   %6phC %8phC\n",
			  context->tx_header[0], context->rx_header,
			  context->rx_header + 6);
	// check crc on header
	recv_crc = unipi_spi_crc(context->rx_header, UNIPI_SPI2_MESSAGE_LEN, 0);
	context->packet_crc =
		*((u16 *)(context->rx_header + UNIPI_SPI2_MESSAGE_LEN));
	if (recv_crc != context->packet_crc) {
		if (((context->rx_header[0] == UNIPI_SPI_OP_IDLE) &&
		     (context->rx_header[2] == 0x55)) ||
	...
	// if (devstatus->opcode == UNIPI_SPI_OP2_IDLE) devstatus->opcode =
	// UNIPI_SPI_OP_IDLE;
	devstatus->interrupt = context->rx_header[3];

	reg = context->tx_header[1]; //&0x3f;
	reply->opcode = context->tx_header[4];
	// reply->data = context->rx_header + ((reply->opcode==UNIPI_SPI_OP_READBIT)?
	// 12 : 8);
	...
	if (reply_max_count == 0xff)
		...
		reply->ret = context->len |
			     ((reply_max_count - context->len) << 8);
	else
		reply->ret = context->len;

	// if (context->operation_callback) {
	if ((reply->ret > 0) && (context->simple_read) && (context->data)) {
		reply_data = context->rx_header +
			     ((reply->opcode == UNIPI_SPI_OP_READBIT) ? 12 : 8);
...
unipi_spi2_alloc_context(struct spi_device *spi_dev, enum UNIPI_SPI_OP op,
			 uint16_t fastop)
// unsigned int len)
{
 ...
 *		         = 1..32  pro WRITEBITS
 */
static int unipi_spi2_write_simple(struct spi_device *spi_dev,
				   ...
				   OperationCallback cb_function)
{
	...
	unipi_spi_trace_1(spi_dev, "REQ (wr=%4d/%d) %6phC %8phC\n", reg, count,
			  context->tx_header, context->tx_header + 6);
 ...
 *		         = 1..32  pro READBIT
 */
static int unipi_spi2_read_simple(struct spi_device *spi_dev,
				  enum UNIPI_SPI_OP op, unsigned int reg,
				  unsigned int count, u8 *data, void *cb_data,
				  OperationCallback cb_function)
{
	...
	context->simple_len = (op == UNIPI_SPI_OP_READBIT) ?
				      roundup_block(count, 8) :
				      count * 2;
	context->len = count;
	context->data = data;
...
struct cb_multi {
	...
	int (*func_simple)(struct spi_device *, enum UNIPI_SPI_OP, unsigned int,
			   unsigned int, u8 *, void *cb_data,
			   OperationCallback cb_function);
	int ret;
};
...
static void multi_callback(void *cb_data, int result)
{
	...
	cb_multi->count -= 4;
next:
	ret = cb_multi->func_simple(cb_multi->spi_dev, cb_multi->op,
				    cb_multi->reg,
				    min((cb_multi->count), (unsigned int)4),
				    cb_multi->data, cb_multi, multi_callback);
	if (ret != 0) {
		cb_multi->ret = ret;
		goto finish;
	}
...
static int unipi_spi2_read_regs_async(void *self, unsigned int reg,
				      ...
				      OperationCallback cb_function)
{
	...
	//	if (count <= 4)
	//		return unipi_spi2_read_simple(spi_dev, UNIPI_SPI_OP_READREG,
	//reg, count, data, cb_data, cb_function);
	...
	return unipi_spi2_read_simple(spi_dev, UNIPI_SPI_OP_READREG, reg,
				      min((cb_multi->count), (unsigned int)4),
				      data, cb_multi, multi_callback);
}

static int unipi_spi2_write_regs_async(void *self, unsigned int reg,
				       unsigned int count, u8 *data,
				       void *cb_data,
				       OperationCallback cb_function)
{
	...
	// if (count <= 4)
	//	return unipi_spi2_write_simple(spi_dev, UNIPI_SPI_OP_WRITEREG, reg,
...
static int unipi_spi2_write_str_async(void *self, unsigned int port, u8 *data,
				      ...
				      OperationCallback cb_function)
{
	...
	context = unipi_spi2_alloc_context(spi_dev, UNIPI_SPI_OP2_WRITESTR,
					   fastop);
	if (context == NULL)
		return -ENOMEM;
...
static int unipi_spi2_read_str_async(void *self, unsigned int port, u8 *data,
				     unsigned int count, void *cb_data,
				     OperationCallback cb_function)
{
	struct spi_device *spi_dev = (struct spi_device *)self;
	...
	context = unipi_spi2_alloc_context(spi_dev, UNIPI_SPI_OP2_READSTR,
					   fastop);
	if (context == NULL)
		return -ENOMEM;
...
static int unipi_spi2_write_bits_async(void *self, unsigned int reg,
				       unsigned int count, u8 *data,
				       void *cb_data,
				       OperationCallback cb_function)
{
	...
	return unipi_spi2_write_simple(spi_dev, UNIPI_SPI_OP_WRITEBITS, reg,
				       count, data, cb_data, cb_function);
}

/*  Async op for READBIT */
static int unipi_spi2_read_bits_async(void *self, unsigned int reg,
				      unsigned int count, u8 *data,
				      void *cb_data,
				      OperationCallback cb_function)
{
	...
	return unipi_spi2_read_simple(spi_dev, UNIPI_SPI_OP_READBIT, reg, count,
				      data, cb_data, cb_function);
}

static int unipi_spi2_idle_op_async(void *self, void *cb_data,
				    OperationCallback cb_function)
{
	struct spi_device *spi_dev = (struct spi_device *)self;
	struct unipi_spi_context *context;
	context = unipi_spi2_alloc_context(spi_dev, UNIPI_SPI_OP2_IDLE,
					   UNIPI_SPI_OP2_IDLE /*, 0x0e55*/);
	if (context == NULL)
		return -ENOMEM;
	return unipi_spi_exec_context(spi_dev, context, cb_data, cb_function);
}
