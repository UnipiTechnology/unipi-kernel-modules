...
typedef int (*RegsAsync)(void *, unsigned int, unsigned int, u8 *, void *,
			 OperationCallback);
typedef int (*BitsAsync)(void *, unsigned int, unsigned int, u8 *, void *,
			 OperationCallback);
typedef int (*StrAsync)(void *, unsigned int, u8 *, unsigned int, void *,
			OperationCallback);
...
#define unipi_mfd_rx_char(channel, port, ch, remain)                          \
	{                                                                     \
		if ((channel)->rx_char_callback)                              \
			(channel)->rx_char_callback((channel)->rx_self, port, \
						    ch, remain);              \
	}

#define unipi_mfd_int_status(channel, int_status)                       \
	{                                                               \
		if ((channel)->interrupt_status_callback)               \
			(channel)->interrupt_status_callback(           \
				(channel)->interrupt_self, int_status); \
	}

#define unipi_read_str_async(channel, port, data, count, cb_data, cb_function) \
	(channel)->op->read_str_async(channel->proto_self, port, data, count,  \
				      cb_data, cb_function)

#define unipi_write_str_async(channel, port, data, count, cb_data,             \
			      cb_function)                                     \
	(channel)->op->write_str_async(channel->proto_self, port, data, count, \
				       cb_data, cb_function)
...
#define unipi_read_regs_async(channel, reg, count, data, cb_data, cb_function) \
	(channel)->op->read_regs_async((channel)->proto_self, reg, count,      \
				       data, cb_data, cb_function)

#define unipi_write_regs_async(channel, reg, count, data, cb_data,         \
			       cb_function)                                \
	(channel)->op->write_regs_async((channel)->proto_self, reg, count, \
					data, cb_data, cb_function)

#define unipi_read_bits_async(channel, reg, count, data, cb_data, cb_function) \
	(channel)->op->read_bits_async((channel)->proto_self, reg, count,      \
				       data, cb_data, cb_function)

#define unipi_write_bits_async(channel, reg, count, data, cb_data,         \
			       cb_function)                                \
	(channel)->op->write_bits_async((channel)->proto_self, reg, count, \
					data, cb_data, cb_function)
...
static inline struct unipi_channel *
unipi_channel_get(struct unipi_channel *channel)
{
