...
unipi_spi_check_message(struct spi_device *spi,
			struct unipi_spi_context *context)
{
	...
	// check crc on header
	recv_crc = unipi_spi_crc(context->rx_header,
				 UNIPI_SPI_MESSAGE_HEADER_LENGTH, 0);
	context->packet_crc = *(
		(u16 *)(context->rx_header + UNIPI_SPI_MESSAGE_HEADER_LENGTH));
	if (recv_crc != context->packet_crc) {
		unipi_spi_error(
			spi,
			"Incorrect CRC1 (Received: %04x Calculated: %04x)\n",
	...
	if (context->len > 0) {
		if (context->simple_read) {
			...
			recv_crc = unipi_spi_crc(context->rx_header2,
						 context->len, recv_crc);
			context->packet_crc =
				context->rx_header2[context->len] |
				(context->rx_header2[context->len + 1] << 8);
			unipi_spi_trace_1(spi, "Read - %d: %10ph", context->len,
					  context->rx_header2);
		} else if (context->data) {
			recv_crc = unipi_spi_crc(
				context->rx_header2,
				UNIPI_SPI_MESSAGE_HEADER_LENGTH, recv_crc);
			recv_crc = unipi_spi_crc(context->data, context->len,
						 recv_crc);
			context->packet_crc =
				context->data[context->len] |
				(context->data[context->len + 1] << 8);
			unipi_spi_trace_1(spi, "Read - %d: %4ph", context->len,
					  context->rx_header2);
		} else {
			return 0;
		}
		if (recv_crc != context->packet_crc) {
			unipi_spi_error(spi,
					"Incorrect CRC2: %04x COMPUTED: %04x\n",
					context->packet_crc, recv_crc);
			unipi_spi_error(spi, "  len=%d, tx=%4phC rx=%4phC\n",
					context->len, context->tx_header,
					context->rx_header2);
...
static int unipi_spi_parse_frame_common(struct unipi_spi_context *context,
					...
					struct unipi_spi_reply *reply)
{
	...
	devstatus->interrupt = context->rx_header[1] &
			       (~0x80); // mask out v2_enabled flag
	if (devstatus->opcode >= UNIPI_SPI_OP_WRITECHAR &&
	    devstatus->opcode < (UNIPI_SPI_OP_WRITECHAR + 4)) {
		devstatus->remain = (context->rx_header[2] == 0 ?
					     256 :
					     context->rx_header[2]) -
				    1;
	...
	if ((devstatus->opcode == UNIPI_SPI_OP_IDLE) &&
	    (context->rx_header[1] & 0x80))
...
static int unipi_spi_parse_frame_simple(struct unipi_spi_context *context,
					struct unipi_spi_devstatus *devstatus,
					struct unipi_spi_reply *reply)
{
	u16 recv_crc;
	...
	// Check second message crc
	recv_crc = unipi_spi_crc(context->rx_header2, context->len,
				 context->packet_crc);
	context->packet_crc = context->rx_header2[context->len] |
			      (context->rx_header2[context->len + 1] << 8);
	// unipi_spi_trace_1(spi, "Read - %d: %10ph", context->len,
	// context->rx_header2);
	...
	if ((reply->ret > 0) && (context->simple_read) && (context->data))
		memmove(context->data,
			context->rx_header2 + UNIPI_SPI_MESSAGE_HEADER_LENGTH,
			context->simple_len);
	return 0;
}

static int unipi_spi_parse_frame_read(struct unipi_spi_context *context,
				      struct unipi_spi_devstatus *devstatus,
				      struct unipi_spi_reply *reply)
{
	u16 recv_crc;
	...
	// Check second message crc
	recv_crc = unipi_spi_crc(context->rx_header2,
				 UNIPI_SPI_MESSAGE_HEADER_LENGTH,
				 context->packet_crc);
	recv_crc = unipi_spi_crc(context->data, context->len, recv_crc);
	context->packet_crc = context->data[context->len] |
			      (context->data[context->len + 1] << 8);
	// printk("Read - %d: %10ph ccrc=%04x rcrc=%04x\n", context->len,
	// context->rx_header2, recv_crc, context->packet_crc);
...
static int unipi_spi_parse_frame_write(struct unipi_spi_context *context,
				       struct unipi_spi_devstatus *devstatus,
				       struct unipi_spi_reply *reply)
{
	int ret = unipi_spi_parse_frame_common(context, devstatus, reply);
...
static struct unipi_spi_context *
unipi_spi_alloc_context(struct spi_device *spi_dev, enum UNIPI_SPI_OP op,
			unsigned int len, unsigned int reg, int add_trans)
{
	...
	context = kzalloc(sizeof(struct unipi_spi_context) +
				  trans_count * sizeof(struct spi_transfer),
			  GFP_ATOMIC);
	if (!context) {
		return NULL;
	}
	...
	context->packet_crc = unipi_spi_crc_set(
		context->tx_header, UNIPI_SPI_MESSAGE_HEADER_LENGTH, 0);

	s_trans = (struct spi_transfer *)(context + 1);
	spi_message_init_with_transfers(&context->message, s_trans,
					trans_count);
 ...
 *		         = 1..32  pro WRITEBITS
 */
static int unipi_spi_write_simple(struct spi_device *spi_dev,
				  enum UNIPI_SPI_OP op, unsigned int reg,
				  unsigned int count, u8 *data, void *cb_data,
				  OperationCallback cb_function)
{
	...
	len = ((op == UNIPI_SPI_OP_WRITEREG) ? count :
					       roundup_block(count, 16)) *
	      2;
	...
	memmove(context->tx_header2 + UNIPI_MODBUS_HEADER_SIZE, data,
		(op == UNIPI_SPI_OP_WRITEREG) ? count * 2 :
						roundup_block(count, 8));
	context->packet_crc = unipi_spi_crc_set(context->tx_header2, len,
						context->packet_crc);
 ...
 *		         = 1..32  pro READBIT
 */
static int unipi_spi_read_simple(struct spi_device *spi_dev,
				 enum UNIPI_SPI_OP op, unsigned int reg,
				 unsigned int count, u8 *data, void *cb_data,
				 OperationCallback cb_function)
{
	...
	len = ((op == UNIPI_SPI_OP_READREG) ? count :
					      roundup_block(count, 16)) *
	      2;
	len += UNIPI_MODBUS_HEADER_SIZE;
	context = unipi_spi_alloc_context(spi_dev, op, len, reg, 1);
	...
	s_trans[2].rx_buf = context->rx_header2;
	s_trans[2].tx_buf =
		context->tx_header2; /* firmware misbehave else can be NULL*/

	context->len = len;
	context->simple_read = 1;
	context->simple_len = (op == UNIPI_SPI_OP_READREG) ?
				      count * 2 :
				      roundup_block(count, 8);
	context->data = data;
...
static int unipi_spi_read_regs_async(void *proto_self, unsigned int reg,
				     ...
				     OperationCallback cb_function)
{
	...
	if (count <= 2) {
		return unipi_spi_read_simple(spi_dev, UNIPI_SPI_OP_READREG, reg,
					     count, data, cb_data, cb_function);
	}
	len = (count * 2);
	data_trans =
		roundup_block(len + UNIPI_SPI_CRC_LENGTH, NEURONSPI_MAX_TX);
	context = unipi_spi_alloc_context(spi_dev, UNIPI_SPI_OP_READREG,
					  len + UNIPI_MODBUS_HEADER_SIZE, reg,
					  1 + data_trans);
	if (context == NULL)
		return -ENOMEM;

	/* crc must be calculated due to firmware behavior */
	context->packet_crc = unipi_spi_crc(context->tx_header2,
					    UNIPI_MODBUS_HEADER_SIZE,
					    context->packet_crc);
	context->packet_crc = unipi_spi_crc_set(data, len, context->packet_crc);

	s_trans = (struct spi_transfer *)(context + 1);
	s_trans[1].delay.value =
		n_spi->internal_delay + ((count > 27) ? (count - 27) : 0);
	s_trans[1].delay.unit = SPI_DELAY_UNIT_USECS;
	...
	for (i = 0; i < data_trans; i++) {
		...
		s_trans[3 + i].tx_buf =
			data + (NEURONSPI_MAX_TX *
				i); /* firmware misbehave else can be NULL*/
		s_trans[3 + i].len =
			(remain > NEURONSPI_MAX_TX) ? NEURONSPI_MAX_TX : remain;
...
static int unipi_spi_write_regs_async(void *proto_self, unsigned int reg,
				      unsigned int count, u8 *data,
				      void *cb_data,
				      OperationCallback cb_function)
{
	...
	if (count <= 2) {
		return unipi_spi_write_simple(spi_dev, UNIPI_SPI_OP_WRITEREG,
					      reg, count, data, cb_data,
					      cb_function);
	}
	len = (count * 2);
	data_trans =
		roundup_block(len + UNIPI_SPI_CRC_LENGTH, NEURONSPI_MAX_TX);
	context = unipi_spi_alloc_context(spi_dev, UNIPI_SPI_OP_WRITEREG,
					  len + UNIPI_MODBUS_HEADER_SIZE, reg,
					  1 + data_trans);
	if (context == NULL)
		return -ENOMEM;
...
static int unipi_spi_write_str_async(void *spi, unsigned int port, u8 *data,
				     unsigned int count, void *cb_data,
				     OperationCallback cb_function)
{
	...
	if (count == 1) {
		context = unipi_spi_alloc_context(
			spi_dev, UNIPI_SPI_OP_WRITECHAR, data[0], port << 8, 0);
		if (context == NULL)
			return -ENOMEM;
		return unipi_spi_exec_context(spi_dev, context, cb_data,
					      cb_function);
	}
	...
	data_trans =
		roundup_block(len + UNIPI_SPI_CRC_LENGTH, NEURONSPI_MAX_TX);
	context = unipi_spi_alloc_context(spi_dev, UNIPI_SPI_OP_WRITESTR,
					  (count & 0xff), port << 8,
					  data_trans);
	if (context == NULL)
		return -ENOMEM;
...
static int unipi_spi_read_str_async(void *spi, unsigned int port, u8 *data,
				    unsigned int count, void *cb_data,
				    OperationCallback cb_function)
{
	int len, data_trans, i, remain;
	...
	data_trans =
		roundup_block(len + UNIPI_SPI_CRC_LENGTH, NEURONSPI_MAX_TX);
	context = unipi_spi_alloc_context(spi_dev, UNIPI_SPI_OP_READSTR,
					  len + UNIPI_MODBUS_HEADER_SIZE,
					  port << 8, 1 + data_trans);
	if (context == NULL)
		return -ENOMEM;
...
static int unipi_spi_write_bits_async(void *proto_self, unsigned int reg,
				      unsigned int count, u8 *data,
				      void *cb_data,
				      OperationCallback cb_function)
{
	...
	if (count == 1) {
		context = unipi_spi_alloc_context(
			spi_dev, UNIPI_SPI_OP_WRITEBIT, data[0], reg, 0);
		if (context == NULL)
			return -ENOMEM;
		return unipi_spi_exec_context(spi_dev, context, cb_data,
					      cb_function);
	}
	return unipi_spi_write_simple(spi_dev, UNIPI_SPI_OP_WRITEBITS, reg,
				      count, data, cb_data, cb_function);
}

/*  Async op for READBIT */
static int unipi_spi_read_bits_async(void *proto_self, unsigned int reg,
				     unsigned int count, u8 *data,
				     void *cb_data,
				     OperationCallback cb_function)
{
	...
	return unipi_spi_read_simple(spi_dev, UNIPI_SPI_OP_READBIT, reg, count,
				     data, cb_data, cb_function);
}

static int unipi_spi_idle_op_async(void *self, void *cb_data,
				   OperationCallback cb_function)
{
	struct spi_device *spi_dev = (struct spi_device *)self;
	struct unipi_spi_context *context;
	context = unipi_spi_alloc_context(spi_dev, UNIPI_SPI_OP_IDLE, 0, 0x0e55,
					  0);
	if (context == NULL)
		return -ENOMEM;
	return unipi_spi_exec_context(spi_dev, context, cb_data, cb_function);
