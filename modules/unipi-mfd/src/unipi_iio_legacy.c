 ...
 * which necessitates using the do_div macro to avoid unnecessary long/long
 * division.
 */
...
static void unipi_iio_stm_read_vref(struct unipi_iio_stm_device *n_iio)
{
	...
	/* ToDo: use do_div */
#ifdef CONFIG_ARM64
	n_iio->kvolt = (((u64)9900 << 2) * vref_int * (10000 + err1)) / 10000 /
		       vref_inp;
	n_iio->fvolt = ((u64)offs1 << 14) / 10;
	n_iio->kamp = (((u64)33000 << 1) * vref_int * (10000 + err2)) / 10000 /
		      vref_inp;
...
#ifdef CONFIG_ARM64
	n_iio->kvolt = (((u64)vref_inp << 16) * 4095 * 10000) / vref_int /
		       (9900 * (10000 + err1));
	n_iio->fvolt = (((u64)vref_inp << 16) * 4095 * offs1) / vref_int / 9900;
	n_iio->kamp = (((u64)vref_inp << 17) * 4095 * 10000) / vref_int /
		      (33000 * (10000 + err2));
	n_iio->famp = (((u64)vref_inp << 17) * 4095 * offs2) / vref_int / 33000;
#else
	/* do_div  - doesn't work in case of big numbers, so we use simplified
   * expression*/
...
/*void unipi_iio_stm_ai_read_voltage(struct iio_dev *iio_dev, struct
iio_chan_spec const *ch, int *val, int *val2, long mask)
{
        ...
        u64 stm_true_val = 0;
        u64 stm_true_ref = 0;
        regmap_read(n_spi->reg_map, n_spi->regstart_table->stm_ai_val_reg,
&stm_ai_val); regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_int,
&stm_v_int_ref); regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_inp,
&stm_v_inp_ref); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ai_vol_err, &stm_v_err); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ai_vol_off, &stm_v_off); stm_true_ref =
((u64)stm_v_int_ref) * 99000; stm_v_inp_ref = stm_v_inp_ref * 10000;
...
*/
/*
void unipi_iio_stm_ai_read_current(struct iio_dev *indio_dev, struct
iio_chan_spec const *ch, int *val, int *val2, long mask)
{
...
&stm_v_int_ref); regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_inp,
&stm_v_inp_ref); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ai_curr_err, &stm_i_err); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ai_curr_off, &stm_i_off); stm_true_ref =
((u64)stm_v_int_ref) * 330000; stm_v_inp_ref = stm_v_inp_ref * 10000;
        stm_true_val = stm_true_ref * ((u64)(stm_ai_val * 1000));
        do_div(stm_true_val, stm_v_inp_ref);
        do_div(stm_true_val, 4096);
...
*/
/*
void unipi_iio_stm_ao_set_voltage(struct iio_dev *indio_dev, struct
iio_chan_spec const *chan, int val, int val2, long mask)
{
        ...
        regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_int,
&stm_v_int_ref); regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_inp,
&stm_v_inp_ref); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ao_vol_err, &stm_v_err); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ao_vol_off, &stm_v_off);

}

void unipi_iio_stm_ao_set_current(struct iio_dev *indio_dev, struct
iio_chan_spec const *chan, int val, int val2, long mask)
{
...
&stm_v_int_ref); regmap_read(n_spi->reg_map, n_spi->regstart_table->vref_inp,
&stm_v_inp_ref); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ao_curr_err, &stm_i_err); regmap_read(n_spi->reg_map,
n_spi->regstart_table->stm_ao_curr_off, &stm_i_off); stm_true_ref =
((u64)stm_v_int_ref) * (330000 + stm_i_err) * 100; stm_v_inp_ref = stm_v_inp_ref
* 1000; stm_true_val = (((stm_true_val * 10000) + (stm_true_val_fraction)) -
stm_i_off) * 4095; do_div(stm_true_ref, stm_v_inp_ref); stm_v_inp_ref =
stm_true_ref; do_div(stm_true_val, stm_v_inp_ref); do_div(stm_true_val, 10); if
(stm_true_val > 4095) stm_true_val = 4095; regmap_write(n_spi->reg_map,
n_spi->regstart_table->stm_ao_val_reg, (unsigned int)stm_true_val);
}
*/
static int unipi_iio_stm_ai_read_raw(struct iio_dev *indio_dev,
				     ...
				     int *val2, long mask)
{
	...
	case 0: {
		if (ch->type == IIO_VOLTAGE) {
			...
			*val = (stm_val < 0x7fff) ? ((stm_val * n_iio->kvolt +
						      n_iio->fvolt) >>
						     14) :
						    0;
			return IIO_VAL_INT;
...
static int unipi_iio_stm_ao_read_raw(struct iio_dev *indio_dev,
				     struct iio_chan_spec const *ch, int *val,
				     int *val2, long mask)
{
	struct unipi_iio_stm_device *n_iio = iio_priv(indio_dev);
...
static int unipi_iio_stm_ao_write_raw(struct iio_dev *indio_dev,
				      struct iio_chan_spec const *ch, int val,
				      int val2, long mask)
{
	struct unipi_iio_stm_device *n_iio = iio_priv(indio_dev);
	...
	case 1: {
		if (ch->type == IIO_CURRENT) {
			if (mask == IIO_CHAN_INFO_RAW) {
				...
				stm_val = (((u64)val * 1000 + val2 / 1000) *
						   n_iio->kamp +
					   n_iio->famp) >>
					  17;
			}
 ...
 ************************************/
static ssize_t
mode_voltage_current_resistance_show(struct device *dev,
...
static ssize_t
mode_voltage_current_resistance_store(struct device *dev,
...
static DEVICE_ATTR(mode_voltage_current, 0660,
		   mode_voltage_current_resistance_show,
		   mode_voltage_current_resistance_store);
...
static int unipi_iio_stm_ai_register(struct device *dev, struct regmap *map,
				     int group_index, int valreg, int modereg)
{
	struct iio_dev *iio_dev;
	struct unipi_iio_stm_device *n_iio;

	iio_dev =
		devm_iio_device_alloc(dev, sizeof(struct unipi_iio_stm_device));
...
static int unipi_iio_stm_ao_register(struct device *dev, struct regmap *map,
				     int group_index, int valreg, int modereg,
				     int r_valreg)
{
	struct iio_dev *iio_dev;
