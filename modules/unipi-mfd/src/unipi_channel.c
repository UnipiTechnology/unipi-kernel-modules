...
{
	struct unipi_channel_sync_cb_data *unipi_cb_data =
			  ...
			  unsigned int count, u8 *data)
{
	...
	cb_data.done = &done;
	ret = unipi_write_bits_async(channel, reg, count, data, &cb_data,
				     unipi_channel_sync_op_callback);
	if (ret != 0)
		return ret;
			 ...
			 unsigned int count, u8 *data)
{
	...
	init_completion(&done);
	cb_data.done = &done;
	ret = unipi_read_bits_async(channel, reg, count, data, &cb_data,
				    unipi_channel_sync_op_callback);
	if (ret != 0)
		return ret;
	wait_for_completion(&done);
			  ...
			  unsigned int count, u16 *data)
{
	...
	init_completion(&done);
	cb_data.done = &done;
	ret = unipi_write_regs_async(channel, reg, count, (u8 *)data, &cb_data,
				     unipi_channel_sync_op_callback);
	if (ret != 0)
		return ret;
	wait_for_completion(&done);
			 ...
			 unsigned int count, u16 *data)
{
	...
	init_completion(&done);
	cb_data.done = &done;
	ret = unipi_read_regs_async(channel, reg, count, (u8 *)data, &cb_data,
				    unipi_channel_sync_op_callback);
	if (ret != 0)
		return ret;
	wait_for_completion(&done);
...
int unipi_ping_sync(struct unipi_channel *channel)
{
	...
	init_completion(&done);
	cb_data.done = &done;
	ret = unipi_ping_async(channel, &cb_data,
			       unipi_channel_sync_op_callback);
	if (ret != 0)
		return ret;
	wait_for_completion(&done);
...
int unipi_channel_init(struct unipi_channel *channel, struct device *dev)
{
	...
	if (dev->of_node) {
		of_property_read_u32(dev->of_node, "modbus-address",
				     &modbus_address);
		if (modbus_address > 254) {
			dev_warn(
				dev,
				"DT property modbus-address value (%d) invalid.\n",
				modbus_address);
			modbus_address = 0;
		}
	} else {
		for (modbus_address = 254; modbus_address > 0;
		     modbus_address--) {
			if (unipi_modbus_dev_by_address(modbus_address) != NULL)
				break;
		}
	}
	...
	if (modbus_address) {
		channel->chrdev =
			unipi_modbus_classdev_register(channel, modbus_address);
	} else {
		dev_warn(
			dev,
			"Undefined modbus-address. Channel inaccessible via modbus chrdev.\n");
	}
	...
	if (nc) {
		...
		iogroup = register_iogroup_device(channel, modbus_address,
						  "unipi-mfd");
		if (IS_ERR(iogroup)) {
		}
	}
