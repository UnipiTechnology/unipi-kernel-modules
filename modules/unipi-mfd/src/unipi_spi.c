...
static ssize_t messages_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
...
static ssize_t errors_tx_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t messages_prio_show(struct device *dev,
...
static ssize_t errors_crc1_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t errors_crc2_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t errors_opcode1_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t firmware_in_progress_show(struct device *dev,
...
static ssize_t hmode_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t hmode_store(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	...
	if (kstrtouint(buf, 0, &val) >= 0) {
		if (val == 0 || val == 1) {
			n_spi->hmode = val;
			n_spi->channel.op = (val == 1) ? &spi_op_v2 :
							 &spi_op_v1;
		}
	}
	return count;
...
static ssize_t edge_delay_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t edge_delay_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
...
static ssize_t internal_delay_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi);
...
static ssize_t internal_delay_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct spi_device *spi = container_of(dev, struct spi_device, dev);
...
static struct attribute *unipi_spi2_attrs[] = {
	&dev_attr_messages.attr,       &dev_attr_errors_tx.attr,
	&dev_attr_bytes.attr,	       &dev_attr_messages_prio.attr,
	&dev_attr_errors_crc1.attr,    &dev_attr_errors_crc2.attr,
	&dev_attr_errors_opcode1.attr, NULL,
};
...
static struct attribute *unipi_spi1_attrs[] = {
	&dev_attr_frequency.attr,      &dev_attr_firmware_in_progress.attr,
	&dev_attr_hmode.attr,	       &dev_attr_edge_delay.attr,
	&dev_attr_internal_delay.attr, NULL,
};
...
int unipi_spi_set_v1(struct spi_device *spi_dev)
{
	...
	if (n_spi->channel.op->write_bits_async(spi_dev, 1007, 1, (u8 *)&data,
						NULL, NULL) == 0) {
		n_spi->hmode = 0;
...
static void try_v2_callback(void *cb_data, int result)
{
	...
	if (result == 2) {
		...
		while (!list_empty(&n_spi->queue)) {
			msg = list_first_entry(&n_spi->queue,
					       struct spi_message, queue);
			list_del_init(&msg->queue);
			context = ((container_of(
				(msg), struct unipi_spi_context, message)));
			if (context->operation_callback)
				context->operation_callback(
					context->operation_callback_data,
					-EAGAIN);
			kfree(context);
		}
...
int unipi_spi_try_v2(struct spi_device *spi_dev)
{
	...
	if (n_spi->enable_v2) {
		...
		if (spi_op_v1.write_bits_async(spi_dev,
					       UNIPI_MFD_COIL_PROTOCOL_MODE - 1,
					       2, (u8 *)&data, (void *)spi_dev,
					       try_v2_callback) != 0) {
			n_spi->enable_v2 = 1;
		}
	}
...
static void unipi_spi_check_busy_queue(struct unipi_spi_device *n_spi)
{
	...
	while (!list_empty(&n_spi->queue)) {
		...
		msg = list_first_entry(&n_spi->queue, struct spi_message,
				       queue);
		list_del_init(&msg->queue);
		context = ((container_of((msg), struct unipi_spi_context,
					 message)));
		ret = spi_async(n_spi->spi_dev, msg);
		if (ret == -EBUSY) {
			context->busy_repeat++;
			if (context->busy_repeat <
			    UNIPI_SPI_BUSY_REPEAT_COUNT) {
				list_add(&context->message.queue,
					 &n_spi->queue);
				hrtimer_start_range_ns(&n_spi->frame_timer,
						       UNIPI_SPI_BUSY_REPEAT_NS,
		...
		if (ret != 0) {
			...
			if (context->operation_callback)
				context->operation_callback(
					context->operation_callback_data, ret);
			kfree(context);
		} else {
...
/* callback of inter-frame timer. Try to insert waiting messages from
 * n_spi->queue */

static enum hrtimer_restart unipi_spi_timer_func(struct hrtimer *timer)
{
	struct unipi_spi_device *n_spi =
		((container_of((timer), struct unipi_spi_device, frame_timer)));
	unsigned long flags;
...
static void unipi_spi_op_complete(void *arg)
{
	...
	if (context->message.status != 0) {
		...
		unipi_spi_trace(spi,
				"Unsuccessful spi transaction: opcode:%d\n",
				context->tx_header[0]);

	} else if (context->parse_frame(context, &devstatus, &reply) != 0) {
		n_spi->stat.errors_crc1++;
		if (!n_spi->probe_mode)
			unipi_spi_error(context->message.spi,
					"Incorrect CRC: opcode:%02x %8ph\n",
					context->tx_header[0],
					context->rx_header);
		...
		case UNIPI_SPI_OP_WRITECHAR:
			...
			unipi_mfd_rx_char(&n_spi->channel, devstatus.port,
					  devstatus.ch, devstatus.remain);
			fallthrough;
		case UNIPI_SPI_OP2_IDLE:
		case UNIPI_SPI_OP_IDLE:
			if (devstatus.interrupt != 0)
				unipi_mfd_int_status(&n_spi->channel,
						     devstatus.interrupt);
			break;
		default:
			reply.ret = -EIO;
		...
		if (reply.ret == -ENXIO) {
			...
			// context->packet_crc, recv_crc); unipi_spi_error(spi, "  len=%d,
			// tx=%4phC rx=%4phC\n", context->len,
			// context->tx_header,context->rx_header2);
    ...
    (context->data)) memmove(context->data, reply.data, context->simple_len);
    //			} else if (context->tx_header[0] ==
    UNIPI_SPI_OP_READSTR) {
    //				// combine len + remain
    ...
    */
	...
	if (context->operation_callback)
		context->operation_callback(context->operation_callback_data,
					    reply.ret);
	kfree(context);
}

int unipi_spi_exec_context(struct spi_device *spi_dev,
			   struct unipi_spi_context *context, void *cb_data,
			   OperationCallback cb_function)
{
	...
	if (ret) {
		if (ret == -EBUSY) {
			...
			n_spi->stat.errors_tx++;
			hrtimer_start_range_ns(&n_spi->frame_timer,
					       UNIPI_SPI_BUSY_REPEAT_NS, 5000,
					       HRTIMER_MODE_REL);
			unipi_spi_trace(spi_dev,
					"Busy. repeat request txopcode:%d\n",
					context->tx_header[0]);
			ret = 0;
		} else {
			n_spi->busy = 0;
...
void unipi_spi_populated(void *self)
{
	...
	struct unipi_spi_device *n_spi = spi_get_drvdata(spi_dev);
	if (spi_dev->max_speed_hz)
		n_spi->frequency = spi_dev->max_speed_hz;
}
...
static int unipi_spi_probe(struct spi_device *spi)
{
...
#else
	hrtimer_setup(&n_spi->frame_timer, unipi_spi_timer_func,
	...
	if (spi->dev.of_node) {
		u32 _val = 0;
		of_property_read_u32(spi->dev.of_node, "probe-always-succeeds",
				     &(probe_always_succeeds));
		of_property_read_u32(spi->dev.of_node, "allow-protocol-v2",
				     &(allow_v2));
		of_property_read_u32(spi->dev.of_node, "edge-delay", &(_val));
		if (_val)
			n_spi->edge_delay = _val;
		_val = 0;
		of_property_read_u32(spi->dev.of_node, "internal-delay",
				     &(_val));
		if (_val)
			n_spi->internal_delay = _val;
	} else {
	...
	// Throw away the first message - the associated SPI Master is sometimes not
	// properly initialised at this point
	...
	for (i = 0; i < 5; i++) {
		...
		/*
    int x= unipi_read_regs_sync(&n_spi->channel, UNIPI_MFD_REG_FW_VERSION, 5,
    first_probe); if (x==5) break; first_probe[0] = 0;
    */
	}
	if (alive > 1) {
	...
	/*
          if (first_probe[0] == 0x0600) {
                  degraded = 1;
                  dev_warn(&spi->dev, "spi device is in degraded (bootloader)
  mode. Load operational firmware!\n"); } else if (first_probe[0] != 0) {
                  dev_info(&spi->dev, "spi channel is alive\n");
          } else if (probe_always_succeeds) {
                  // dummy channel
                  dev_info(&spi->dev, "spi channel is forced up\n");
...
struct unipi_channel *unipi_spi_get_channel(struct spi_device *spi_dev)
{
	...
	if ((spi_dev == NULL) || (spi_dev->dev.driver == NULL) ||
	    (strcmp(unipi_spi_driver.driver.name, spi_dev->dev.driver->name) !=
	     0))
		return NULL;
